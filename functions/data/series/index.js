...
export async function onRequest(context) {
  ...
  const config = await (
    await env.ASSETS.fetch("https://bigsolo.org/data/config.json")
  ).json();
  const seriesJsons = config.LOCAL_SERIES_FILES;
  const series = await Promise.all(
    seriesJsons.map(async (seriesFile) => {
      const res = await env.ASSETS.fetch(
        "https://bigsolo.org/data/series/" + seriesFile
      );
      ...
      const lastEpisode =
        seriesData.episodes && seriesData.episodes.length > 0
          ? {
              number:
  ...
  const enrichedReco = await Promise.all(
    recoList.map(async (reco) => {
      const res = await env.ASSETS.fetch(
        "https://bigsolo.org/data/series/" + reco.file
      );
      const seriesData = await res.json();
      const baseSeries = series.find(
        (s) => slugify(s.title) === slugify(seriesData.title)
      );
      ...
      return {
        ...
        character_image:
          reco.characterImage ||
          `/img/reco/${reco.file.replace(".json", ".png")}`,
        color: reco.color,
      };
    })
  );
